...
def _run_async(coro):
    """Run async coroutine, handling existing event loops (Databricks/Jupyter).
    ...
    # Loop already running (Databricks/Jupyter)
    # Try nest_asyncio first (allows nested event loops)
    try:
        import nest_asyncio
        nest_asyncio.apply()
        return loop.run_until_complete(coro)
    except ImportError:
        # nest_asyncio not available, use thread pool executor
...
def chunk_document_sync(
    ...
    Returns:
        List of ProcessedChunk objects.
    """
    return _run_async(chunk_document(document, config))
...
def process_documents_sync(
    ...
    Returns:
        Tuple of (documents, all_chunks).
    """
    return _run_async(process_documents(html_files, chunk_config))
