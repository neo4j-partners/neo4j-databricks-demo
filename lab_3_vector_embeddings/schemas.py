...
class EmbeddingConfig(BaseModel):
    ...
    Attributes:
        ...
        endpoint: Optional endpoint URL for Databricks Foundation Model.
    """
    ...
    endpoint: Optional[str] = Field(
        default=None, description="Endpoint URL for Databricks Foundation Model"
    )
...
class SearchConfig(BaseModel):
    ...
    @classmethod
    def validate_alpha_for_linear(cls, v: Optional[float], info: Any) -> Optional[float]:
        """Ensure alpha is provided when using linear ranker."""
        ranker = info.data.get("ranker")
